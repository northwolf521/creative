...
extern "C" {
     ...
     * @brief 设置 tbus 选项
     *
     * @param[in] a_iOptionName    此参数指定需要设置的tbus选项，目前不支持任何选项。
     *
     * @param[in] a_pvOptionValue 相应选项的具体设置值，目前不支持任何选项。
...
#define tbus_get_channel_outmsg_num tbus_channel_get_outmsg_num
    ...
    /**
     * @brief 初始化TBUSRECV类型的变量
     *
     * @param a_pstRecv 指向需要初始化的变量
     * @note 定义TBUSRECV类型的变量后，必须先调用 tbus_init_recv 接口初始化这个变量，然后再设置关注的字段值
     * @see tbus_recv_ex
     */
    TSF4G_API int tbus_init_recv(INOUT LPTBUSRECV a_pstRecv);
...
#define tbus_set_pkg_header    tbus_set_bus_head
     ...
     *
     * @param[in] a_iHandle tbus handle, acquired by tbus_new()
     * @param[in] a_iSrc 源bus地址，不支持特殊地址
     * @param[in] a_iDst 目的bus地址，不支持特殊地址
     * @param[out] a_pfUsed 获取到的队列空间使用比例，0~1之间的浮点型。不能为NULL
     *
     * @retval 0    SUCCESS
     *         !0   FAILED
     */
    TSF4G_API int tbus_get_channel_recvside_used_proportion(IN const int a_iHandle, IN TBUSADDR a_iSrc, IN TBUSADDR a_iDst,
                                                            OUT float* a_pfUsed);
     ...
     *         !0   FAILED
     */
    TSF4G_API int tbus_get_channel_sendside_used_proportion(IN const int a_iHandle, IN TBUSADDR a_iSrc, IN TBUSADDR a_iDst,
                                                            OUT float* a_pfUsed);
     ...
     *         !0   FAILED
     */
    TSF4G_API int tbus_empty_channel_recvside(IN const int a_iHandle, IN TBUSADDR a_iSrc, IN TBUSADDR a_iDst);
     ...
     *         !0   FAILED
     */
    TSF4G_API int tbus_empty_channel_sendside(IN const int a_iHandle, IN TBUSADDR a_iSrc, IN TBUSADDR a_iDst);
    /** @} */
