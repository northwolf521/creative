...
agent_server::agent_server() : _listen(NULL), _redis(NULL) {
...
int32_t agent_server::init(int32_t argc, char** argv) {
    ...
    INFO("agent_server::init() succ");
    return 0;
}

int32_t agent_server::final() {
    server_node_base::final();
    INFO("final() succ");
    return 0;
}

void agent_server::signal_usr1() {
    INFO("signal_usr1() succ");
}

void agent_server::signal_usr2() {
    INFO("signal_usr2() succ");
}

void agent_server::signal_int() {
    INFO("signal_int() succ");
    stop();
}

conn_listen* agent_server::listen_ptr() {
    return _listen;
}

void agent_server::tick_proc() {
    if (!redis()) {
...
redis_proxy* agent_server::redis() {
    return _redis;
}

void agent_server::reload() {
...
int32_t main(int32_t argc, char** argv) {
    ...
    agent_server* svr = new agent_server();
